...
deploy_monitoring_to_container() {
    ...
    echo "📦 Deploying monitoring to CT $container_id ($name)..."
...
check_memory_usage() {
    ...
    if [ "\$mem_total" -gt 0 ]; then
        ...
        if [ "\$usage_percent" -ge "\$MEMORY_CRITICAL_THRESHOLD" ]; then
            echo "🚨 CRITICAL: Memory usage \${usage_percent}% >= \${MEMORY_CRITICAL_THRESHOLD}%"
            return 2
        elif [ "\$usage_percent" -ge "\$MEMORY_WARNING_THRESHOLD" ]; then
            echo "⚠️ WARNING: Memory usage \${usage_percent}% >= \${MEMORY_WARNING_THRESHOLD}%"  
            return 1
        else
            echo "✅ Memory usage normal: \${usage_percent}%"
            return 0
        fi
    fi
}
...
check_disk_usage() {
    ...
    if [ "\$disk_usage" -ge 90 ]; then
        ...
        echo "⚠️ WARNING: Disk usage \${disk_usage}% >= 75%"
        return 1
    else
...
check_load_average() {
    ...
    if [ "\$load_percent" -ge 200 ]; then
        echo "🚨 CRITICAL: Load ratio \${load_percent}% >= 200%"
        return 2
    elif [ "\$load_percent" -ge 150 ]; then
        echo "⚠️ WARNING: Load ratio \${load_percent}% >= 150%"
        return 1
    else
        echo "✅ Load average normal: \${load_percent}%"
        return 0
    fi
}
...
main() {
    echo "🔍 Container Health Check: CT \$CONTAINER_ID (\$CONTAINER_NAME)"
    echo "📊 Type: \$CONTAINER_TYPE | Priority: \$PRIORITY"
    echo "⏰ Timestamp: \$(date)"
    echo ""
    ...
    echo "📋 Overall Status:"
    case \$exit_code in
        0) echo "✅ All systems normal" ;;
        1) echo "⚠️ Warning conditions detected" ;;
        2) echo "🚨 Critical conditions detected" ;;
    esac
...
EOF
    ...
    echo "   📤 Copying monitoring script to container via API..."
...
async def deploy_monitoring():
    ...
    try:
        ...
        if response.status_code != 200:
            print('❌ Failed to create monitoring directory')
            return False
        ...
        if response.status_code != 200:
            print('❌ Failed to write monitoring script')
            return False
        ...
        if response.status_code != 200:
            print('⚠️ Script deployed but chmod may have failed')
            return True  # Still consider success
        
        print('✅ Monitoring script deployed successfully')
        ...
        if response.status_code == 200:
            print('✅ Automated monitoring configured (every 5 minutes)')
        else:
            print('⚠️ Cron configuration may have failed')
        
        return True
        
    except Exception as e:
        print(f'❌ Error deploying monitoring: {e}')
        return False
    finally:
        await client.aclose()
...
" 2>/dev/null)
    ...
    if [ $deploy_result -eq 0 ]; then
        ...
        echo "   🧪 Testing monitoring script via API..."
        python3 -c "
import asyncio
import httpx
...
async def test_monitoring():
    ...
    try:
        ...
        if response.status_code == 200:
            print('✅ Monitoring script test completed')
        else:
            print('⚠️ Script test may have issues')
    except:
        print('⚠️ Could not test monitoring script')
    finally:
        await client.aclose()
...
echo "🚀 Deploying Container Monitoring Scripts"
echo "=========================================="
...
" 2>/dev/null)
    ...
    if [ "$status" = "running" ]; then
        ...
        echo "📦 CT $container_id: Skipped (status: stopped)"
    elif [ "$status" = "not_found" ]; then
        echo "📦 CT $container_id: Not found"
    else
        echo "📦 CT $container_id: Error checking status ($status)"
    fi
    echo ""
done

echo "📊 Deployment Summary:"
echo "  ✅ Successful: $successful_deployments/$total_deployments"
echo "  📈 Success Rate: $((successful_deployments * 100 / total_deployments))%"

if [ "$successful_deployments" -eq "$total_deployments" ]; then
    echo ""
    echo "🎉 All monitoring scripts deployed successfully!"
    echo "📊 Containers now monitored every 5 minutes"
    echo "📝 Logs available at: /var/log/health_check.log (in each container)"
    echo "🔍 Check status: pct exec [CONTAINER_ID] -- tail -f /var/log/health_check.log"
else
    echo ""
    echo "⚠️ Some deployments failed. Check container status and permissions."
fi
